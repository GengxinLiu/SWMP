...
def single_transformation_verifier(dataset, model, inlier_th):
    ...
    res = dataset['target'].T - model['scale'] * np.matmul(model['rotation'], dataset['source'].T) - model[
...
def joint_transformation_estimator(dataset, best_inliers=None):
    ...
    joint_points0 = np.ones_like(
        np.linspace(0, 1, num=np.min((source0.shape[0], source1.shape[0])) + 1)[1:].reshape((-1, 1))) * dataset[
                        'joint_direction'].reshape((1, 3))
    ...
    if not isalternate:
        ...
        res = least_squares(objective_eval, np.hstack((rotvec0, rotvec1)), verbose=0, ftol=1e-4, method='lm',
                            args=(source0_centered, target0_scaled_centered, source1_centered, target1_scaled_centered,
                                  joint_points0, False))
...
def joint_transformation_verifier(dataset, model, inlier_th):
    # dataset: dict, fields include source, target, nsource, ntarget
    # model: dict, fields include rotation, scale, translation
    res0 = dataset['target0'].T - model['scale0'] * np.matmul(model['rotation0'], dataset['source0'].T) - model[
        'translation0'].reshape((3, 1))
    inliers0 = np.sqrt(np.sum(res0 ** 2, 0)) < inlier_th
    res1 = dataset['target1'].T - model['scale1'] * np.matmul(model['rotation1'], dataset['source1'].T) - model[
        'translation1'].reshape((3, 1))
...
if __name__ == '__main__':
    ...
    # file_name = my_dir + '/results/test_pred/pickle{}/{}_{}_{}_rt_pn.pkl'.format(save_exp, args.domain, args.nocs,
    #                                                                              args.item)
    file_name = my_dir + '/results/pickle/{}/{}_{}_{}_rt_pn.pkl'.format(save_exp, args.domain, args.nocs,
                                                                        args.item)  # fix by gengxin
    all_rts = {}
    ...
    for i in range(len(test_group)):
        try:
            ...
            for j in range(num_parts):
                ...
                print('part %d -- rdiff: %f degree, tdiff: %f, sdiff %f, ninliers: %f, npoint: %f' % (
                    j, rdiff, tdiff, sdiff, np.sum(best_inliers), best_inliers.shape[0]))
                target0_fit = best_model['scale'] * np.matmul(best_model['rotation'], source0.T) + best_model[
                    'translation'].reshape((3, 1))
