...
if __name__ == '__main__':
    ...
    parser.add_argument('--item', default='scissors', help='object category for benchmarking')
    ...
    for i in pbar:
        ...
        try:
            ...
            for key in ['baseline', 'nonlinear']:
                ...
                # save bboxs3d_rt add by gengxin
                save_box_dir = my_dir + '/results/gx_bboxs3d_rt/{}'.format(main_exp)
                os.makedirs(save_box_dir, exist_ok=True)
                save_box_path = os.path.join(save_box_dir, basename + ".json")
                bboxs3d_rt_save = copy.deepcopy(bboxs3d_rt)
                bboxs3d_rt_save['iou'] = []
                for j in range(0, num_parts):
                    bboxs3d_rt_save['gt'][j] = bboxs3d_rt_save['gt'][j].tolist()
                    bboxs3d_rt_save['pred'][j] = bboxs3d_rt_save['pred'][j].tolist()
                    bboxs3d_rt_save['iou'].append(iou_per_part[j])
                json.dump(bboxs3d_rt_save, open(save_box_path, "w"))
