...
if __name__ == '__main__':
    ...
    parse.add_argument("--dataset", type=str, choices=["shape2motion", "partnet2shapemotion", "partnet"])
    ...
    base_path = my_dir  # with a lot of different objects, and then 0001/0002/0003/0004/0005
    # >>>>>>>>>>>>>>>>>>>>>>>> config end >>>>>>>>>>>>>>>>>>>>>>>>>>>>#
    ...
    for obj_name in all_objs:
        ...
        for sub_dir in selected_objs:  # regular expression
            #
            sub_dir = sub_dir.replace("\\", "/")  # fix by gengxin
            ...
            with open(json_name[0]) as json_file:
                ...
                for i in range(num - 1):
                    ...
                    while joint_dict[child_name] != 'dof_rootd':
                        print('joint {} now looking at child {}, has parent {}'.format(i, child_name,
                                                                                       joint_dict[child_name]))
                ...
                for i in range(num):
                    ...
                    if dof_name == 'dof_rootd':
                        ...
                        origin = SubElement(visual, 'origin', rpy="0.0 0.0 0.0",
                                            xyz="{} {} {}".format(links_pos[i][0], links_pos[i][1], links_pos[i][2]))
                ...
                for i in range(num - 1):
                    ...
                    origin = SubElement(joints[i], "origin",
                                        xyz="{} {} {}".format(joints_pos[i][0], joints_pos[i][1], joints_pos[i][2]),
                                        rpy="0 0 0")
                    # we may need to change the joint name
                    if joints_type[i] == 'revolute':
                        axis = SubElement(joints[i], "axis", xyz="{} {} {}".format(joints_axis[i][0], joints_axis[i][1],
                                                                                   joints_axis[i][2]))
                        limit = SubElement(joints[i], "limit", effort="1.0", lower="-3.1415", upper="3.1415",
                                           velocity="1000")
                    if joints_type[i] == 'prismatic':
                        axis = SubElement(joints[i], "axis", xyz="{} {} {}".format(joints_axis[i][0], joints_axis[i][1],
                                                                                   joints_axis[i][2]))
                        limit = SubElement(joints[i], "limit", effort="1.0", lower="0", upper="0.4", velocity="1000")
