...
class PointNet(nn.Module):
    def __init__(self, emb_dims=256, output_channels=40):
        super(PointNet, self).__init__()
        # self.args = args
        # emb_dims = 256
        # self.conv1 = nn.Conv1d(3, 64, kernel_size=1, bias=False)
        self.conv1 = nn.Conv1d(4, 64, kernel_size=1, bias=False)  # xyz + point-indicator
...
class DGCNN_cls(nn.Module):
    def __init__(self, emb_dims=512, output_channels=128):
    ...
    def forward(self, x):
        ...
        idx = knn(x[:, :3, :], k=self.k)  
        # (batch_size, 4, num_points) -> (batch_size, 4*2, num_points, k)
        ...
        # (batch_size, 64+64+128, num_points)
        x = torch.cat((x1, x2, x3), dim=1)
        # x = torch.cat((x1, x3, x4), dim=1)  # (batch_size, 64+128+256, num_points)
...
class SiameseNet(nn.Module):
    def __init__(self, embeding_net, embed_dim, dropout=0.5):
        super(SiameseNet, self).__init__()
        self.embeding_net = embeding_net
    ...
    def forward(self, pts1, pts2):
        B = pts1.shape[0]
        feat1 = self.embeding_net(pts1).reshape(B, -1)  # B x C
        feat2 = self.embeding_net(pts2).reshape(B, -1)  # B x C
        # print(feat1.shape, feat2.shape)
        feat = torch.cat([feat1, feat2], dim=-1)  # B x 2C
        out = self.fc(feat)
